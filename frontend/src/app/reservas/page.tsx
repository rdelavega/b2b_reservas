      ...
      <div className="flex items-center justify-between">
        <div>
          <p className="font-mono text-xs uppercase tracking-[0.3em] text-line">
            Libro de reservas{esDemo ? " · muestra" : ""}
          </p>
        ...
        {reservas.map((r) => (
          <li key={r.id} className="ticket flex overflow-hidden">
            <div className="flex w-28 shrink-0 flex-col items-center justify-center gap-1 border-r-2 border-dashed border-ink/20 bg-paper-dim py-4 font-mono">
              <span className="text-[10px] uppercase tracking-widest text-ink/40">{r.fecha.slice(5, 10)}</span>
              <span className="text-lg font-semibold text-ink">{r.horaInicio}</span>
              <span className="text-[10px] uppercase tracking-widest text-ink/40">{r.numPersonas} pax</span>
            </div>
            <div className="flex flex-1 items-center justify-between px-5 py-4">
              <div>
                <p className={`font-mono text-[10px] uppercase tracking-widest ${ESTADO_COLOR[r.estado]}`}>
                  {r.estado}
                </p>
                <p className="mt-1 font-display text-lg italic text-ink">Mesa reservada</p>
              </div>
              <div className="flex gap-4 font-mono text-xs uppercase tracking-widest">
                <Link href={`/reservas/${r.id}/editar`} className="text-onyx transition hover:text-line">
                  Editar
                </Link>
                <Link href={`/reservas/${r.id}/cancelar`} className="text-stamp transition hover:text-line">
                  Cancelar
                </Link>
              </div>
            </div>
